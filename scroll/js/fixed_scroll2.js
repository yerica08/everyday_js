...
일반적으로 0.1초에서 몇 초까지 설정할 수 있어.
너무 짧거나 긴 값은 부자연스러운 애니메이션을 만들 수 있어.
ease:

GSAP에서 제공하는 다양한 이징 함수가 있어. 예를 들어, "linear", "power1.in", "power2.out" 등 여러 옵션이 있어.
이 값은 애니메이션의 속도 변화를 조절해.
autoAlpha:

0 (완전히 투명)에서 1 (완전히 불투명)까지의 값을 가질 수 있어.
yPercent:

-100%에서 100%까지의 값을 가질 수 있어. 예를 들어, 0%는 원래 위치, -100%는 위로 100% 이동, 100%는 아래로 100% 이동을 의미해.
stagger:

각 요소가 애니메이션을 시작하는 간격을 설정해. 일반적으로 0초 이상 설정할 수 있어. */
